...
function mostrarServicios(serviciosMostrar) {
  ...
  // Delegamos listeners de editar/eliminar
  setTimeout(() => {
...
function normalizarDuracion(valor) {
  // Acepta number|string y devuelve number o null si no es válido
  if (valor == null) return null; // null o undefined
...
function abrirModalCrear() {
  ...
  // Reset select a placeholder (recomendado al crear)
...
function editarServicio(id) {
  ...
  // Asegurar que el select tenga la opción existente
  const sel = document.getElementById('duracion');
...
async function guardarServicio() {
  ...
  try {
    ...
    const servicioData = {
      ...
      // Enviamos número válido; si por alguna razón viene vacío, no mandamos undefined/NaN
...
function limpiarFormularioServicio() {
  ...
  // Dejar placeholder en el select al cerrar
  const sel = document.getElementById('duracion');
  sel.value = '';
}
